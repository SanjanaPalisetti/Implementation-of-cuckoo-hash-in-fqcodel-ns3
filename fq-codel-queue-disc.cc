...
TypeId FqCoDelQueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::FqCoDelQueueDisc")
    ...
    .AddAttribute ("DropBatchSize",
                   ...
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("CuckooHash", "Variable to use cuckoo hash", BooleanValue (false),
              MakeBooleanAccessor (&FqCoDelQueueDisc::m_cuckooHash), MakeBooleanChecker ());
  ;
  return tid;
}
...
FqCoDelQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
  ...
  if(m_cuckooHash)
  {
    ...
    // getting index of bucket 

  	uint32_t h = ret % m_flows;
  	uint32_t h1 = (ret / m_flows) % m_flows;
  	...
  	int32_t ret1;
  	uint32_t Q_Num1,Q_Num;

  	// if it belongs to the flow in table 1

  	if(m_flowsIndices.find (h) != m_flowsIndices.end ()){
  		NS_LOG_DEBUG ("Matching flow found in table 1 at index " << h);
  		flow1 = StaticCast<FqCoDelFlow> (GetQueueDiscClass (m_flowsIndices[h]));
  		item1 = GetQueueDiscClass (m_flowsIndices[h])->GetQueueDisc()->Peek();
  		ret1 = Classify (item);
  		if(ret == ret1){
  	...
  	// if it belongs to the flow in table 2

  	else if(m_flowsIndices1.find (h1) != m_flowsIndices1.end ()){
  		NS_LOG_DEBUG ("Matching flow found in table 2 at index " << h1);
  		flow1 = StaticCast<FqCoDelFlow> (GetQueueDiscClass (m_flowsIndices1[h1]));
  		item1 = GetQueueDiscClass (m_flowsIndices1[h1])->GetQueueDisc()->Peek();
  		ret1 = Classify (item);
  		if(ret == ret1){
  			flow->GetQueueDisc ()->Enqueue (item);
  		}
  	...
  	// there are no matching flows in the tables

  	else{

  		// if bucket isnt occupied

  		if (m_flowsIndices.find (h) == m_flowsIndices.end ()){
	      NS_LOG_DEBUG ("No collisions, creating a new flow queue in table 1 with index " << h);
    	...
    	// if there is bound to be at least one collision

  		else{

  			// iterating to handle multiple colliosions
  			uint32_t i;
  			for(i=0;i<m_flows;i++){

  				// when collision is in table 1

  				if(i%2==0){

  					// if bucket is empty (to place displaced element)

		  			if(m_flowsIndices.find(h) == m_flowsIndices.end()){
		  				NS_LOG_DEBUG ("After " << i << " collision(s), creating a new flow queue in table 1 with index " << h);
		  				m_flowsIndices[h]=Q_Num1; //Q_Num1 is set while displacing in previous collision
		  				break;
			  		...
			  		else if(m_flowsIndices.find(h) != m_flowsIndices.end()){

			  			// if occupying flow is inactive

			  			flow = StaticCast<FqCoDelFlow> (GetQueueDiscClass (m_flowsIndices[h]));
			  			if (flow->GetStatus () == FqCoDelFlow::INACTIVE){
			  				if(i == 0){
				  				NS_LOG_DEBUG ("After " << i << " collision(s), using inactive flow queue in table 1 with index " << h);
				  				flow->SetStatus (FqCoDelFlow::NEW_FLOW);
				                flow->SetDeficit (m_quantum);
				                m_newFlows.push_back (flow);
				                flow->GetQueueDisc ()->Enqueue (item);
				            }
				            m_flowsIndices[h] = Q_Num1;
			                break;
			            }

			            // if occupying flow in not inactive

			            else if (flow->GetStatus () != FqCoDelFlow::INACTIVE){

			            	Q_Num1 = m_flowsIndices[h];

			  	  			if(i==0){
				  	  			NS_LOG_DEBUG ("Creating a new flow queue replacing old with index " << h);
						        flow1 = m_flowFactory.Create<FqCoDelFlow> ();
						        Ptr<QueueDisc> qd = m_queueDiscFactory.Create<QueueDisc> ();
						        qd->Initialize ();
						        flow1->SetQueueDisc (qd);
						        AddQueueDiscClass (flow1);
						        m_flowsIndices[h] = GetNQueueDiscClasses () - 1;
					        	flow1->SetStatus (FqCoDelFlow::NEW_FLOW);
					        	flow1->SetDeficit (m_quantum);
					        	m_newFlows.push_back (flow1);
						        flow1->GetQueueDisc ()->Enqueue (item);
					    	}

					        item1=GetQueueDiscClass (m_flowsIndices[h])->GetQueueDisc()->Peek();
					        ret=Classify();
					        h1 = ( ret / m_flows ) % m_flows;
					        if(i!=0){
					        	m_flowsIndices[h] = Q_Num;
					        	NS_LOG_DEBUG ("Collision number " << i << " : at table 1 with index " << h << " : " << Q_Num1 << "--->" << Q_Num);
					        }
					    }
					}
				}

				// when collision is in table two

				else{

					// if bucket is empty (to place displaced element)

					if(m_flowsIndices1.find(h1) == m_flowsIndices1.end()){
						NS_LOG_DEBUG ("After " << i << " collision(s), creating a new flow queue in table 2 with index " << h1);
		  				m_flowsIndices1[h1]=Q_Num;
		  				break;
					...
					//m_flowsIndices1 this map need to define in fq-codel-queue-disc.h ? 
						...
						flow = StaticCast<FqCoDelFlow> (GetQueueDiscClass (m_flowsIndices1[h1]));

			  			// if occupying flow is inactive

			  			if (flow->GetStatus () == FqCoDelFlow::INACTIVE){
			  				m_flowsIndices1[h1] = Q_Num;
			  				NS_LOG_DEBUG ("After " << i << " collision(s), replacing inactive flow queue in table 2 with index " << h);
			                break;
			            }
			            ...
			            if (flow->GetStatus () != FqCoDelFlow::INACTIVE){
			            	item1 = GetQueueDiscClass (m_flowsIndices1[h1])->GetQueueDisc()->Peek();
					          ret = Classify(item1);
					          h = ret % m_flows;
			            	Q_Num = m_flowsIndices1[h1];//save q number that is being replaced
					          m_flowsIndices1[h1] = Q_Num1;//replacing with  previously saved queue
					          NS_LOG_DEBUG ("Collision number " << i << " : at table 2 with index " << h << " : " << Q_Num << "--->" << Q_Num1);
					    }
					}
				}
			}
